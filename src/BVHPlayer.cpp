...
#define CAMERA_MOVE_SPEED 0.3

static float camera_yaw = 90.0f;
static float camera_pitch = 70.0f;
static float camera_distance = 5.0f;
static double cameraLookCenter_x = 0.0f;
static double cameraLookCenter_y = 0.0f;
static double cameraLookCenter_z = 0.0f;
...
void display(void) {
    ...
    if (!bvh)
        gluLookAt(1.0, 1.0, -camera_distance, 0, 0, 0, 0, 1, 0);
    else {
        double tmpX = (sin(camera_pitch * (M_PI / 180)) * cos(camera_yaw * (M_PI / 180))) * camera_distance +
                      cameraLookCenter_x;
        double tmpZ = (sin(camera_pitch * (M_PI / 180)) * sin(camera_yaw * (M_PI / 180))) * camera_distance +
                      cameraLookCenter_z;
        double tmpY = (cos(camera_pitch * (M_PI / 180))) * camera_distance + cameraLookCenter_y;

        gluLookAt(tmpX, tmpY, tmpZ, cameraLookCenter_x, cameraLookCenter_y, cameraLookCenter_z,
                  0, 1, 0);
    }
...
void motion(int mx, int my) {
    // camera rotation
    {
        if (drag_mouse_r) {
            ...
            if (camera_pitch < 10.0)
                camera_pitch = 10.0;
            else if (camera_pitch > 180.0)
                camera_pitch = 180.0;
        }
        if (drag_mouse_l) {
...
void keyboard(unsigned char key, int mx, int my) {
    ...
    {
        if (key == 'w') { // forward
            cameraLookCenter_x -= (sin(camera_pitch * (M_PI / 180)) * cos(camera_yaw * (M_PI / 180))) * CAMERA_MOVE_SPEED;
            cameraLookCenter_z -= (sin(camera_pitch * (M_PI / 180)) * sin(camera_yaw * (M_PI / 180))) * CAMERA_MOVE_SPEED;
            cameraLookCenter_y -= (cos(camera_pitch * (M_PI / 180))) * CAMERA_MOVE_SPEED;
        }
        if (key == 'a') { // left
            cameraLookCenter_x -= (sin(camera_pitch * (M_PI / 180)) * sin(camera_yaw * (M_PI / 180))) * CAMERA_MOVE_SPEED;
            cameraLookCenter_z += (sin(camera_pitch * (M_PI / 180)) * cos(camera_yaw * (M_PI / 180))) * CAMERA_MOVE_SPEED;
        }
        if (key == 's') { // back
            cameraLookCenter_x += (sin(camera_pitch * (M_PI / 180)) * cos(camera_yaw * (M_PI / 180))) * CAMERA_MOVE_SPEED;
            cameraLookCenter_z += (sin(camera_pitch * (M_PI / 180)) * sin(camera_yaw * (M_PI / 180))) * CAMERA_MOVE_SPEED;
        ...
        if (key == 'h'){ // reset camera pos
            cameraLookCenter_x = 0;
            cameraLookCenter_z = 0;
            cameraLookCenter_y = 0;
        }
    }
